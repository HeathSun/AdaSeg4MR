...
def callback(recognizer, audio):
    ...
    logging.info('Audio data written to %s', prompt_audio_path)
        
    prompt_text = wav_to_text(prompt_audio_path)
    logging.info('Transcribed text: %s', prompt_text)
    
    clean_prompt = extract_prompt(prompt_text, wake_word)
    logging.info('Extracted prompt: %s', clean_prompt)
    
    if clean_prompt:
        print(f"Wake word detected, extracted prompt: {clean_prompt}")  # changed code
        logging.info('Processing user prompt: %s', clean_prompt)
        call = function_call(clean_prompt)
        logging.info('Function call determined: %s', call)
