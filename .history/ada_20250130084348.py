...
# A flag to control interruption
...
def speak(text):
    ...
    with openai_client.audio.speech.with_streaming_response.create(
        ...
        for chunk in response.iter_bytes(chunk_size=1024):
            if interrupt_speaking:  # 如果被打断，跳出语音播放
                break
            if stream_start:
...
def start_segmentation():
    ...
    results = segmentation_model.predict(0, save=False, show=True, conf=0.15, verbose=False)  # Suppress output if supported
    
def callback(recognizer, audio):
    global interrupt_speaking
    if interrupt_speaking:  # 如果正在输出，允许打断
        interrupt_speaking = False  # 立即中断
    ...
    if clean_prompt:
        ...
        interrupt_speaking = True  # 设置为True开始新的语音输出
        speak(response)

def start_listening():
    global interrupt_speaking
    while True:
        if not interrupt_speaking:  # 只有在没有正在输出语音时才监听
            ...
            if clean_prompt:
                ...
                response = groq_prompt(prompt=clean_prompt, img_context=visual_context)
                print(f'Ada: {response}')
                interrupt_speaking = True  # 开始新的语音输出
                speak(response)
