...
def speak(text):
    ...
    with openai_client.audio.speech.with_streaming_response.create(
        ...
        speaking_status = True  # Mark that the system is speaking
        for chunk in response.iter_bytes(chunk_size=1024):
            if stream_start:
                ...
                if amplitude > silence_threshold:
                    player_stream.write(chunk)
                    stream_start = True
        speaking_status = False  # Mark that the system is no longer speaking
...
def callback(recognizer, audio):
    if speaking_status:
        # If speaking, don't process the new audio input yet
        return
