...
def start_segmentation():
    ...
    def segmentation_task():
        while segmentation_active.is_set():
            results = segmentation_model.predict(0, save=False, show=False, conf=0.15)
            time.sleep(1)  # Adjust the sleep time as needed

    executor.submit(segmentation_task)
    
def start_segmentation_in_new_console():
    subprocess.Popen(
        ["start", "cmd", "/k", "python", "-c", "import ada; ada._run_segmentation_logic()"],
        shell=True
    )

def stop_segmentation():
    if segmentation_active.is_set():
        segmentation_active.clear()
        print("Segmentation stopped.")
        speak("Segmentation has been stopped.")
    else:
        print("Segmentation is not currently running.")
        speak("Segmentation is not currently running.")
...
def callback(recognizer, audio):
    ...
    if clean_prompt:
        ...
        if 'stop segmentation' in clean_prompt.lower():
            stop_segmentation()
            return
