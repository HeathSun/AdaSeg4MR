...
sys_msg = (
    'You are a multi-modal AI voice assistant named Ada, after the British computer scientist Ada Turing,'
...
def segmentation_loop():
    speak("Segmentation running in the background.")
    while SEGMENTATION_RUNNING:
        ...
        if results:
            ...
            speak("No objects detected in the frame.")
    speak("Segmentation stopped.")

def start_segmentation():
    global SEGMENTATION_THREAD, SEGMENTATION_RUNNING
    if SEGMENTATION_RUNNING:
        print("Segmentation is already running.")
        return
...
def stop_segmentation():
    global SEGMENTATION_RUNNING
    SEGMENTATION_RUNNING = False
    print("Segmentation stop signal sent.")
...
def callback(recognizer, audio):
    ...
    if clean_prompt:
        ...
        if 'take screenshot' in call:
            ...
            start_segmentation()
            visual_context = None
        elif 'stop segmentation' in call or 'halt segmentation' in call:
            stop_segmentation()
            visual_context = None
