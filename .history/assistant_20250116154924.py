...
sys_msg = (
    'You are a multi-modal AI voice assistant named Alan, after the British computer scientist Alan Turing,'
...
def function_call(prompt):
    sys_msg = (
        ...
        'respond with only one selection from this list: ["extract clipboard", "segmentation", "take screenshot", "capture webcam", "None"] \n'
...
def start_segmentation():
    ...
    speak("I am performing the image segmentation now")
    # Perform real-time segmentation
    results = segmentation_model.predict(0, save=True, show=True, conf=0.15)
    
    # Extract detected instances and counts
    object_counts = {}
    for result in results:
        for obj in result.boxes.cls:
            obj_name = segmentation_model.names[int(obj)]
            object_counts[obj_name] = object_counts.get(obj_name, 0) + 1
    
    # Create the response string
    response = "I am doing segmentation and have divided these instances: " + \
